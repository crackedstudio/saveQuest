...
use starknet::storage::{
    ...
    fn join_vault(ref self: ContractState) -> bool {
        ...
        assert(_token_instace.balance_of(_caller ) >= self.contribution_amount.read(), 'Insufficient balance');

        // transfer funds to the vault
        assert(_token_instace.transfer_from(_caller, _contract_address, _contribution_amount), 'Transfer failed');
        ...
        if (self.current_participants.read() == self.max_participants.read()) {
            // start yeild generation logic
        }
