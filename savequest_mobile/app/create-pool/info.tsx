...
import { usePoolCreate, TokenType, GroupType } from '@/context/PoolCreateContext'
...
export default function CreatePoolInfo() {
  ...
  const proceed = () => {
    setState((s) => ({ ...s, poolName: name.trim(), token, groupType }))
    ...
    <SafeAreaView className="w-full h-full bg-primary">
      <ScrollView className="p-[24px]" contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View className="bg-bg rounded-2xl p-[20px]">
          ...
          <View className="flex flex-row gap-x-3">
            {(['USDT','USDC','BTC'] as TokenType[]).map((t) => (
              <TouchableOpacity key={t} onPress={() => setToken(t)} className={`flex-1 h-[70px] rounded-xl items-center justify-center ${token === t ? 'bg-secondary' : 'bg-highlight'}`}>
        ...
        <TouchableOpacity onPress={proceed} className="bg-secondary rounded-2xl mt-8 h-[56px] items-center justify-center">
          <Text className="text-black text-[18px] font-extrabold">NEXT: ADD MEMBERS</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  )
